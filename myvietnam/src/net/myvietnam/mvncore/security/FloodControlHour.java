/*
 * $Header: /cvsroot/mvnforum/myvietnam/src/net/myvietnam/mvncore/security/FloodControlHour.java,v 1.1 2009/08/19 10:46:41 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.1 $
 * $Date: 2009/08/19 10:46:41 $
 *
 * ====================================================================
 *
 ...
 */
public class FloodControlHour {

    private static final Logger log = LoggerFactory.getLogger(FloodControlHour.class);
     ...
     * @see FloodControlHour#reachMaximum(Integer, String)
     */
...
class ControlledAction {
    ...
    private synchronized void removeTimeoutControlledIP() {
        ...
        if ((now - lastRemoveTime) > FloodControlHour.REMOVE_INTERVAL) {
            lastRemoveTime = now;
    ...
    private synchronized void removeTimeoutActions() {
        ...
        if (now - lastRemoveTime > FloodControlHour.REMOVE_INTERVAL) {
            ...
            for (Iterator iter = actionHistoryList.iterator(); iter.hasNext(); ) {
                 ...
                 */
                if ( (currentAction == null) || ((now - currentAction.longValue()) > DateUtil.HOUR)) {
                    iter.remove();
                }
            } //for
        }
