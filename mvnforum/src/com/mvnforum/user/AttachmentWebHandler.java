/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/user/AttachmentWebHandler.java,v 1.145 2010/08/17 04:44:33 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.145 $
 * $Date: 2010/08/17 04:44:33 $
 *
 * ====================================================================
 *
...
public class AttachmentWebHandler {
    ...
    private static final Logger log = LoggerFactory.getLogger(AttachmentWebHandler.class);
    ...
    /*
     * @todo find a way to cache the file based on the HTTP protocol
     * @todo check permission
     */
    public void downloadAttachment(GenericRequest request, HttpServletResponse response)
        throws BadInputException, DatabaseException, ObjectNotFoundException, IOException, AuthenticationException  {
        ...
        */
        ...
        try {
            ...
            try {
                if (thumbnail) {
                    ImageUtil.createThumbnail(inputStream, outputStream, MVNForumConfig.getThumbnailWidth(), MVNForumConfig.getThumbnailHeight(), locale);
                } else {
    ...
    /**
     * NOTE: This method should be called before any attempt to delete a post
    ...
    static void deleteAttachments_inPost(int postID) throws DatabaseException {
        ...
        // now delete files on disk and Lucene index
    ...
    /**
     * NOTE: This method should be called before any attempt to delete a thread
    ...
    static void deleteAttachments_inThread(int threadID) throws DatabaseException {
        ...
        for (Iterator iter = attachmentBeans.iterator(); iter.hasNext(); ) {
            ...
            // Note that, we could re-write so we could make only one call to delete all attachments in a thread
            try {
                DAOFactory.getAttachmentDAO().delete(attachID);
            } catch (Exception ex) {
    ...
    public void prepareListAttachments(GenericRequest request, GenericResponse response)
        ...
        permission.ensureCanGetAttachmentInAtLeastOneForum();
    ...
    public void processSearchAttachments(GenericRequest request, GenericResponse response)
        ...
        CategoryBuilder builder = categoryBuilderService.getCategoryTreeBuilder();
        CategoryTree tree = new CategoryTree(builder);
        CategoryTreeListener listener = categoryService.getManagementCategorySelector(request, response, "searchattachments", CategoryTreeListener.NOT_SHOW_FULL_CATEGORY);
        tree.addCategeoryTreeListener(listener);
        request.setAttribute("Result", tree.build());
