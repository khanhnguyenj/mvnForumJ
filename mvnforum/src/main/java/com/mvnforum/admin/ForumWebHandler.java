/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/admin/ForumWebHandler.java,v 1.95 2009/05/04 08:44:11 lexuanttkhtn Exp $
 * $Author: lexuanttkhtn $
 * $Revision: 1.95 $
...
public class ForumWebHandler {
    ...
    public void prepareAdd(GenericRequest request, GenericResponse response)
        ...
        CategoryTreeListener listener = categoryService.getManagementCategorySelector(request, response, true, null, CategoryTreeListener.SHOW_FULL_CATEGORY);
    ...
    public void processAdd(GenericRequest request, GenericResponse response)
        ...
        try {
            ...
            String localizedMessage = MVNForumResourceBundle.getString(locale, "mvncore.exception.BadInputException.forumname_exists", new Object[] {forumName});
    ...
    public void prepareEdit(GenericRequest request, GenericResponse response)
        ...
        CategoryBuilder builder = categoryBuilderService.getCategoryTreeBuilder();
        CategoryTree tree = new CategoryTree(builder);
        CategoryTreeListener listener = categoryService.getManagementCategorySelector(request, response, forumBean, CategoryTreeListener.SHOW_FULL_CATEGORY);
        tree.addCategeoryTreeListener(listener);
        request.setAttribute("Result", tree.build());
    }
    ...
    public void processUpdate(GenericRequest request, GenericResponse response)
        ...
        // Clear post cache because if a forum's status is changed, its threads will be changed -> must reload threads. 
        PostCache.getInstance().clear();
    }
