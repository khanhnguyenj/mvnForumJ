/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/user/UserModuleProcessor.java,v 1.208
 * 2010/06/17 09:50:27 minhnn Exp $ $Author: minhnn $ $Revision: 1.208 $ $Date: 2010/06/17 09:50:27
 * $
 *
 * ====================================================================
 *
...
public class UserModuleProcessor implements ModuleProcessor {
  ...
  private static final Logger log = LoggerFactory.getLogger(UserModuleProcessor.class);

  protected static final String ORIGINAL_REQUEST = "mvnforum.mobile.OriginalRequest";
  ...
  public String process(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (requestURI.equals("/index")) {
        if (MVNForumConfig.getEnablePortalLikeIndexPage()) {
          forumWebHandler.prepareListIndex(genericRequest, genericResponse, requestURI);// no
                                                                                        // permission
        } else {
        ...
        threadWebHandler.prepareListRecentThreads_limit(genericRequest, genericResponse);// no
                                                                                         // permission
        ...
        threadWebHandler.prepareMergeThread(genericRequest, genericResponse);
      } else if (requestURI.equals("/mergethreadprocess")) {
        threadWebHandler.processMergeThread(genericRequest);
        // zt.add.end.080917
        ...
        ajaxValidator.validate(genericRequest, genericResponse);
        return null;
        ...
        memberWebHandler.processUploadAvatar(mainServlet.getServletConfig(), genericRequest);
      } else if (requestURI.equals("/updateavatar")) {
        memberWebHandler.processUpdateMemberAvatar(genericRequest);
        ...
        memberWebHandler.processGuestSetting(genericRequest, genericResponse);
      ...
      if (e instanceof BadInputException) {
        ...
        log.error(
            "Exception in UserModuleProcessor [" + e.getClass().getName() + "] : " + e.getMessage(),
            e);
      }
      requestURI = "/error";
    ...
    try {
      ...
      Action action = new ActionInForumUserModule(genericRequest, requestURI);// may throw
