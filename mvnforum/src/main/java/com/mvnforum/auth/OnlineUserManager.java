/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/auth/OnlineUserManager.java,v 1.111 2010/08/10 11:52:50 minhnn Exp $
 * $Author: minhnn $
 * $Revision: 1.111 $
...
public final class OnlineUserManager {
    ...
    public static final String PASSWORD_OF_METHOD_OPENID        = "OpenID"; //must not be changed in all cases
     ...
     * MemberMatkhau   for password
     */
    public void processLogin(HttpServletRequest request, HttpServletResponse response, boolean checkPost)
        throws AuthenticationException, DatabaseException, BadInputException, FloodException {
    ...
    public void processLogin(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            Integer maxWrongLogins = new Integer(FloodControlHour.getActionsPerHour(MVNForumGlobal.FLOOD_ID_LOGIN_PER_IP));
     ...
     * NOTE: the parameter response can be null
     */
    ...
    private OnlineUser login(GenericRequest request, GenericResponse response,
                               String memberName, String memberPassword, boolean passwordEncoded)
        throws AuthenticationException, DatabaseException {
    ...
    public void deleteCookie(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (MVNForumConfig.getEnableDeleteCookie() == false) {
            throw new IllegalStateException("Cannot run this because MVNForumConfig.getEnableDeleteCookie() is false.");
        }
    ...
    public OnlineUser getOnlineUser(HttpServletRequest request)
        ...
        if (user == null) {
            ...
            if ((user == null) && enableLoginInfoInOpenID) {
                String memberName = ParamUtil.getAttribute(session, MVNFORUM_SESSION_OPENID_USERNAME);// this method never return null
                //System.out.println("OnlineUserManager.getOnlineUser() (user is null) for OpenID memberName = " + memberName);
                if (memberName.length() > 0) {
                    ...
                    try {
                        user = login(request, null, memberName, OnlineUserManager.PASSWORD_OF_METHOD_OPENID, true);
            ...
            if (enableLoginInfoInOpenID) {
                String memberName = ParamUtil.getAttribute(session, MVNFORUM_SESSION_OPENID_USERNAME);// this method never return null
                //System.out.println("OnlineUserManager.getOnlineUser() (user not null) for OpenID memberName = " + memberName);
                if (memberName.length() > 0) {
                    String identity = ParamUtil.getAttribute(session, MVNFORUM_SESSION_OPENID_IDENTITY);
                    session.setAttribute(MVNFORUM_SESSION_OPENID_USERNAME, null);
                    session.setAttribute(MVNFORUM_SESSION_OPENID_IDENTITY, null);
