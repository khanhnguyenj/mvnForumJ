<%--
 - $Header: /cvsroot/mvnforum/mvnforum/srcweb/mvnplugin/mvnforum/admin/editmembergroup.jsp,v 1.27 2009/10/23 08:40:37 trungth Exp $
 - $Author: trungth $
 - $Revision: 1.27 $
 - $Date: 2009/10/23 08:40:37 $
 -
 - ====================================================================
 -
...
<br />
<%
MemberGroupBean memberGroupBean = (MemberGroupBean) request.getAttribute("MemberGroupBean");
...
<div class="nav center">
  ...
  <fmt:message key="mvnforum.admin.editmembergroup.title" />
</div>
<br />
...
<form action="<%=urlResolver.encodeURL(request, response,"editmembergroupprocess", URLResolverService.ACTION_URL)%>" method="post" name="submitform" onsubmit="return false;">
<%=urlResolver.generateFormAction(request, response, "editmembergroupprocess")%>
<mvn:securitytoken />
<input type="hidden" name="groupid" value="<%=memberGroupBean.getGroupID()%>" />
<input type="hidden" name="memberid" value="<%=memberGroupBean.getMemberID()%>" />
...
<table class="tborder" width="95%" cellspacing="0" cellpadding="3" align="center">
  ...
  <tr class="<mvn:cssrow/>">
    <td valign="top"><fmt:message key="mvnforum.common.members" />:</td>
    <td><%=memberGroupBean.getMemberName()%></td>
  </tr>
  <tr class="<mvn:cssrow/>">
    ...
    <td>
      <select id="Privilege" name="Privilege" size="1">
        ...
        --%>
      </select> <fmt:message key="mvnforum.admin.addmembergroup.help.privilege_for_all" />
    </td>
  </tr>
<%
boolean neverExpire = (memberGroupBean.getCreationDate().compareTo(memberGroupBean.getExpireDate()) == 0);
%>
  <tr class="<mvn:cssrow/>">
    ...
    <td>
      <table class="noborder" cellpadding="3" cellspacing="0" width="100%">
        <tbody>
          <tr>
            <td>
              <input name="expiretype" value="0" class="noborder" type="radio" onclick="chooseExpireType(0);"<%if (neverExpire) {%> checked="checked"<%}%> /> <fmt:message key="mvnforum.common.status.never_expired" />&nbsp;&nbsp;
              <input name="expiretype" value="1" class="noborder" type="radio" onclick="chooseExpireType(1);"<%if (neverExpire == false) {%> checked="checked"<%}%> /> <fmt:message key="mvnforum.common.date.expire_date" />(dd/mm/yyyy)
            </td>
          </tr>
          <tr>
            <td<%if (neverExpire) {%> style="display: none"<%} else {%> style="display:" <%}%> id="chooseexpiredate">
              <%
                if (neverExpire == false) {
                  Timestamp memberExpireDate = memberGroupBean.getExpireDate();
                  request.setAttribute("TimeToDisplay", memberExpireDate);
                }
              %> 
