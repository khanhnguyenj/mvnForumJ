/*
 * $Header: /cvsroot/mvnforum/mvnforum/src/com/mvnforum/admin/MemberGroupWebHandler.java,v 1.58 2009/04/29 07:54:56 lexuanttkhtn Exp $
 * $Author: lexuanttkhtn $
 * $Revision: 1.58 $
 * $Date: 2009/04/29 07:54:56 $
 *
 * ====================================================================
 *
...
public class MemberGroupWebHandler {
    ...
    private static final Logger log = LoggerFactory.getLogger(MemberGroupWebHandler.class);
    ...
    public void processAdd(GenericRequest request)
        throws IOException, BadInputException, CreateException, DatabaseException, AuthenticationException {
        ...
        if ((expireType != 0) && (expireType != 1)) {
            String localizedMessage = MVNForumResourceBundle.getString(locale, "mvncore.exception.BadInputException.expire_date_not_valid");
        ...
        if (expireType != 0) {
            ...
            if (expireDate.compareTo(now) < 0) {
                String localizedMessage = MVNForumResourceBundle.getString(locale, "mvncore.exception.BadInputException.expired_period_not_valid");
                throw new BadInputException(localizedMessage);
            }
    ...
    public void prepareEdit(GenericRequest request)
        ...
        MemberGroupBean memberGroupBean = DAOFactory.getMemberGroupDAO().getMemberGroup(memberID, groupID);
    ...
    public void processUpdate(GenericRequest request)
        ...
        DAOFactory.getMemberGroupDAO().update(memberID, groupID, now/*modifiedDate*/, memberExpireDate);
        String actionDesc = MVNForumResourceBundle.getString(MVNForumConfig.getEventLogLocale(), "mvnforum.eventlog.desc.UpdateMemberGroupProcess", new Object[]{new Integer(groupID), new Integer(memberID)});
        eventLogService.logEvent(onlineUser.getMemberName(), request.getRemoteAddr(),MVNForumConstant.EVENT_LOG_MAIN_MODULE, MVNForumConstant.EVENT_LOG_SUB_MODULE_ADMIN, "update member group", actionDesc, EventLogService.MEDIUM);
    }
